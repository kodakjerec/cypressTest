...
describe('輸入測試', () => {
  const login = () => {
    ...
    if (window.localStorage.getItem('tempToken')) {
      window.localStorage.setItem('MBIS-AUTH_USER', window.localStorage.getItem('tempToken'))
      needLogin = false
    }
    if (needLogin) {
      ...
      const token = window.localStorage.getItem('MBIS-AUTH_USER')
      if (token != null)
  ...
  context('資料輸入', () => {
    ...
    it('被保險人', () => {
      ...
      // 電話
      cy.get('select[name="phoneLight"]').select(testData.insured.phoneLight);
      ...
      cy.get('select[name="phoneNight"]').select(testData.insured.phoneNight);
    ...
    it('要保人', () => {
      ...
      // ***** 以下為要保人輸入 *****
      cy.get('select[name="relation"]').select(testData.proposer.relation);
      cy.get('input[name="infoName"]').type(testData.proposer.infoName);
      cy.get('input[name="infoGender"]').last().click();
      cy.get('input[name="infoPersonId"]').type(testData.proposer.infoPersonId);
      cy.get('input[name="romanName"]').type(testData.proposer.romanName);

      // 日期(切換成成年日期)
      cy.get('.mat-datepicker-input').type(testData.proposer.infoBirthday);

      // 國籍
      cy.get('input[name="infoNationalIsROC"]').check();
      cy.get('select[name="infoNationalOtherDesc"]').select(testData.proposer.infoNational);
      cy.get('select[name="infoMarry"]').select(testData.proposer.infoMarry);
      cy.get('input[name="serviceUnit"]').type(testData.proposer.serviceUnit);
      cy.get('input[name="jobType"]').type(testData.proposer.jobType);
      cy.get('input[name="workDetail"]').type(testData.proposer.workDetail);
      cy.get('input[name="partTimeDetail"]').type(testData.proposer.partTimeDetail);

      // 正職職業代碼 A101
      cy.get('button[name="jobOccupyFull"]').click();
      cy.get('button.btn-info').eq(0).click().wait(200);
      cy.get('tbody > :nth-child(1) > .align-items-center > .form-check-input').click();
      cy.get('.modal-footer > .btn').click();
      ...
      // 電話
      cy.get('select[name="phoneLight"]').select(testData.proposer.phoneLight);
      cy.get('input[name="phoneNumberLightArea"]').type(testData.proposer.phoneNumberLightArea);
      cy.get('input[name="phoneNumberLight"]').type(testData.proposer.phoneNumberLight);
      cy.get('input[name="phoneNumberLightExt"]').type(testData.proposer.phoneNumberLightExt);
      cy.get('select[name="phoneNight"]').select(testData.proposer.phoneNight);
      cy.get('input[name="phoneNumberNightArea"]').type(testData.proposer.phoneNumberNightArea);
      cy.get('input[name="phoneNumberNight"]').type(testData.proposer.phoneNumberNight);
      cy.get('input[name="phoneNumberNightExt"]').type(testData.proposer.phoneNumberNightExt);
      cy.get('select[name="phoneCellphone"]').select(testData.proposer.phoneCellphone);
      cy.get('input[name="cellphoneNumberNation"]').type(testData.proposer.cellphoneNumberNation).trigger('change');
      cy.get('input[name="phoneCellphoneNumber"]').type(testData.proposer.phoneCellphoneNumber);

      // 地址
      cy.get('select[name="addrCity"]').select(testData.proposer.addrCity);
      cy.get('select[name="addrDistrict"]').select(testData.proposer.addrDistrict);
      cy.get('input[name="addrAddress"]').type(testData.proposer.addrAddress);

      cy.get('input[name="emailLocal"]').type(testData.proposer.emailLocal);
      cy.get('input[name="emailDomain"]').type(testData.proposer.emailDomain);
      cy.get('input[name="policyType"]').first().check();
    ...
    it('告知事項', () => {
      ...
      cy.get('input[name="sickName"]').type(testData.sickInfoList.sickName);
      cy.get('input[name="hospital"]').type(testData.sickInfoList.hospital);
      cy.get('input[name="treatmentYear"]').type(testData.sickInfoList.treatmentYear);
      cy.get('input[name="treatmentMonth"]').type(testData.sickInfoList.treatmentMonth);
      cy.get('input[name="treatmentDay"]').type(testData.sickInfoList.treatmentDay);
      cy.get('input[name="treatmentWay"]').type(testData.sickInfoList.treatmentWay);
      cy.get('input[name="treatmentResults"]').type(testData.sickInfoList.treatmentResults);
    ...
    it('疾病問卷', () => {
      ...
      cy.get('input[name="conditions"]').check({ force: true });
          ...
          cy.get('app-accordion-items').eq(i).find('tbody > tr > :nth-child(3)')
                        .then(($parent) => {
                          ...
                          if ($parent.children('.btn-success[style*="visibility: visible"]').length == 0) {
                            ...
                            // 關閉拍照
                            cy.get('app-camera-dialog > .camera > .camera-controls--top > div').first().click().wait(500);

                            // // 切換鏡頭
                            // cy.get('app-camera-dialog > .camera > .camera-controls--top > div').last().click().wait(500);
                            // // 拍照
                            // cy.get('app-camera-dialog > .camera > .camera-controls--bottom > div').first().click().wait(500);
                            // // 確認
                            // cy.get('app-camera-dialog > .camera > .camera-controls--bottom > div').last().click().wait(500);
                            // // success
